...
let SCALE = 0;   // art pixels -> css pixels (0 = not laid out yet)
let DPR = 1;
let TURNED = null;
...
function layout() {
  ...
  // Fill the window: scale continuously to whatever the viewport allows,
  // rather than only stepping at whole numbers. A whole-number-only zoom
  // keeps every art pixel identically sized, but on most windows it leaves
  // large unused borders — the window's aspect ratio is essentially never an
  // exact multiple of the game's. The backing store is sized to match this
  // exact CSS size (times devicePixelRatio) and drawn with image-rendering:
  // pixelated, so edges stay crisp even though pixels are no longer perfectly
  // even multiples of one screen pixel.
  const s = Math.max(0.35, Math.min(availW / BUF.w, availH / BUF.h));
  ...
  canvas.width = Math.round(BUF.w * s * dpr);
  canvas.height = Math.round(BUF.h * s * dpr);
}

/* Switch the canvas into the 960x540 space the text is written against. */
function textSpace() {
  const k = (DPR * SCALE * BUF.w) / VIEW.w;
...
function blitBuffer() {
  ...
  ctx.drawImage(buffer, 0, 0, BUF.w, BUF.h, 0, 0, BUF.w * SCALE, BUF.h * SCALE);
}
